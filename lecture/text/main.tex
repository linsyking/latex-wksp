...
\begin{frame}[fragile]{Highlight, Under/Upper Braces}
    Use the \structure{soul} package to highlight text:
    \begin{command}
        \begin{minted}{latex}
\usepackage{soul}
\hl{Sample Text}
    ...
    \end{command}
    \medskip
    Use \structure{\textbackslash overbrace/\textbackslash underbrace} to add text over/below text with braces.
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item \alert{\textbackslash\textbackslash [}\structure{offset}\alert{]} - begin a new line with an vertical offset, \structure{offset} is the size of needed space  (not recommended, using  \LC{\vspace} instead.)
